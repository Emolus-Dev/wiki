...
export function useTreeDialogs(spaceId, expandedNodes, emit) {
  ...
  async function createDocument(close) {
    ...
    try {
      ...
      const newDocKey = await crStore.createPage(
      ...
      if (!createIsGroup.value && newDocKey) {
        router.push({
          name: "DraftChangeRequest",
          params: { spaceId: spaceId.value, docKey: newDocKey },
        });
      }
    } catch (error) {
      handleError({
        ...
        fallback: __("Error creating draft"),
        notify: toast.error,
      });
    }
  }
  ...
  async function deleteDocument(close) {
    try {
      ...
      handleError({
        error,
        context: "Error creating delete draft",
        fallback: __("Error creating draft"),
        notify: toast.error,
      });
    }
  ...
  async function renameDocument(close) {
    ...
    try {
      ...
      handleError({
        error,
        context: "Error updating title",
        fallback: __("Error updating title"),
        notify: toast.error,
      });
    }
  }
  ...
  async function createExternalLink(close) {
    ...
    try {
      ...
      handleError({
        error,
        context: "Error creating external link",
        fallback: __("Error creating draft"),
        notify: toast.error,
      });
    }
  ...
  async function updateExternalLink(close) {
    ...
    try {
      ...
      handleError({
        error,
        context: "Error updating external link",
        fallback: __("Error updating external link"),
        notify: toast.error,
      });
    }
  }
