<template>
  <div class="wiki-editor-container">
    ...
    <div v-else class="wiki-editor-loading">Loading editor...</div>
...
async function uploadFile(file) {
  try {
    ...
    handleError({
      ...
      fallback: __("Failed to upload file"),
      notify: toast.error,
    });
    throw error;
  }
      ...
      if (file) {
        uploadFile(file)
          ...
          .catch((error) => {
            handleError({
              error,
              context: "Failed to upload pasted image",
              fallback: __("Failed to upload image"),
            });
          });
      }
      return true;
    ...
    if (isImage) {
      uploadFile(file)
        ...
        .catch((error) => {
          handleError({
            error,
            context: "Failed to upload dropped image",
            fallback: __("Failed to upload image"),
          });
        });
...
.wiki-editor-content pre {
  ...
  font-size: 0.875rem;
  line-height: 1.6;
  font-family: "Fira Code", "JetBrains Mono", "SF Mono", Menlo, Monaco,
